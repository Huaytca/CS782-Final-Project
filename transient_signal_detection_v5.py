'''
In this version, we use Bayesian change point detection
'''
import pandas as pd
import numpy as np
...
def improved_bayesian_change_point_detection(signal, window_size=15, overlap=0.5):
    ...
    diff_vector -= 0.01
    print(f'MAX: {np.max(diff_vector)}')
    print(f'MIN: {np.min(diff_vector)}')
    print(f'SHAPE: {diff_vector.shape}')
...
def process_bluetooth_signal(signal):
    start_idx, end_idx, window_sums, diff_vector = improved_bayesian_change_point_detection(
        signal, window_size=150, overlap=0.25) # 150, 0.85; 150, 0.25
...
#     start_idx, end_idx, _, _ = improved_bayesian_change_point_detection(signal, window_size=150, overlap=0.25)
